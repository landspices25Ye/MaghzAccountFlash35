...
export const SupplierStatementReport: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const contacts = (contactsResult.data || []) as Record<string, any>[];

      const rows: SupplierLine[] = [];
      for (const c of contacts) {
        let query = `SELECT * FROM purchase_invoices WHERE supplier_id = $1`;
  ...
  const handleExportExcel = async () => {
    const cols = [
      ...
      { key: 'totalPurchases', header: 'إجمالي المشتريات' },
      { key: 'lastInvoice', header: 'آخر فاتورة' },
    ];
    await exportToExcel(suppliers, cols, 'Supplier_Statement');
  };
  ...
  const handleExportPDF = async () => {
    const cols = [
      ...
      { key: 'totalPurchases', header: 'المشتريات', width: 15 },
    ];
    await exportToPDF(suppliers, cols, 'Supplier_Statement', {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card>
          <div className="p-4 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">{t('reports.totalPurchases')}</p>
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(totalPurchases)}</p>
          </div>
        </Card>
      </div>
        ...
        <div className="p-4">
          <Table
            ...
            columns={[
              ...
              { key: 'totalPurchases', header: 'إجمالي المشتريات', align: 'right', render: (row) => formatCurrency(row.totalPurchases) },
              { key: 'lastInvoice', header: 'آخر فاتورة' },
            ]}
