...
export const salesApi = {
  ...
  async updateCustomer(id: string, data: Partial<Omit<Customer, 'id' | 'companyId'>>): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const fields: string[] = [];
    ...
    values.push(id);
    const result = await adapter.query(`UPDATE customers SET ${fields.join(', ')} WHERE id = $${idx}`, values);
    return { success: result.success, error: result.error };
  },

  async deleteCustomer(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM customers WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },

  async getCustomerStatement(customerId: string): Promise<{ success: boolean; data?: CustomerStatementRow[]; error?: string }> {
    const adapter = await adapterPromise;
    // Invoices as debits, payments as credits (mock via sales_invoices paid_amount for now; real app uses receipts)
    const result = await adapter.query(
  ...
  async getInvoiceById(id: string): Promise<{ success: boolean; data?: SalesInvoice; error?: string }> {
    const adapter = await adapterPromise;
  ...
  async updateInvoice(id: string, data: Partial<Omit<SalesInvoice, 'id' | 'companyId' | 'lines'>> & { lines?: SalesInvoiceLine[] }): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const fields: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
    ...
    if (fields.length > 0) {
      values.push(id);
      await adapter.query(`UPDATE sales_invoices SET ${fields.join(', ')} WHERE id = $${idx}`, values);
    }
    if (data.lines) {
  ...
  async deleteInvoice(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM sales_invoices WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },

  async postInvoice(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query(`UPDATE sales_invoices SET status = 'posted' WHERE id = $1 AND status = 'draft'`, [id]);
    return { success: result.success, error: result.error };
  },
  ...
  async getQuotationById(id: string): Promise<{ success: boolean; data?: Quotation; error?: string }> {
    const adapter = await adapterPromise;
    const res = await adapter.query(`SELECT q.*, c.name as customer_name FROM quotations q LEFT JOIN customers c ON q.customer_id = c.id WHERE q.id = $1 LIMIT 1`, [id]);
  ...
  async createQuotation(data: Omit<Quotation, 'id'>): Promise<{ success: boolean; id?: string; error?: string }> {
    ...
    if (tx.success && tx.results?.[0]?.[0]) {
      const qid = tx.results[0][0].id as string;
      for (const line of data.lines) {
        await adapter.query(`INSERT INTO quotation_lines (quotation_id, product_id, quantity, unit_price, discount_percent, line_total) VALUES ($1,$2,$3,$4,$5,$6)`,
          [qid, line.productId, line.quantity, line.unitPrice, line.discountPercent, line.lineTotal]);
      }
  ...
  async updateQuotation(id: string, data: Partial<Omit<Quotation, 'id' | 'companyId' | 'lines'>> & { lines?: QuotationLine[] }): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const fields: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
    ...
    if (data.status !== undefined) { fields.push(`status = $${idx++}`); values.push(data.status); }
    if (data.notes !== undefined) { fields.push(`notes = $${idx++}`); values.push(data.notes); }
    if (fields.length > 0) { values.push(id); await adapter.query(`UPDATE quotations SET ${fields.join(', ')} WHERE id = $${idx}`, values); }
    if (data.lines) {
  ...
  async deleteQuotation(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM quotations WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },

  async convertQuotationToInvoice(id: string, invoiceData: Omit<SalesInvoice, 'id'>): Promise<{ success: boolean; id?: string; error?: string }> {
    const createRes = await this.createInvoice(invoiceData);
    if (createRes.success) {
      const adapter = await adapterPromise;
      await adapter.query(`UPDATE quotations SET status = 'converted' WHERE id = $1`, [id]);
    }
    return createRes;
  },
  ...
  async getReturnById(id: string): Promise<{ success: boolean; data?: SalesReturn; error?: string }> {
    const adapter = await adapterPromise;
    const res = await adapter.query(
      `SELECT r.*, c.name as customer_name, i.invoice_number as invoice_number_ref FROM sales_returns r LEFT JOIN customers c ON r.customer_id = c.id LEFT JOIN sales_invoices i ON r.invoice_id = i.id WHERE r.id = $1 LIMIT 1`, [id]
    );
    if (!res.success || !res.rows?.[0]) return { success: false, error: res.error || 'Not found' };
  ...
  async updateReturn(id: string, data: Partial<Omit<SalesReturn, 'id' | 'companyId' | 'lines'>> & { lines?: SalesReturnLine[] }): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const fields: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
    ...
    if (data.status !== undefined) { fields.push(`status = $${idx++}`); values.push(data.status); }
    if (data.notes !== undefined) { fields.push(`notes = $${idx++}`); values.push(data.notes); }
    if (fields.length > 0) { values.push(id); await adapter.query(`UPDATE sales_returns SET ${fields.join(', ')} WHERE id = $${idx}`, values); }
    if (data.lines) {
  ...
  async deleteReturn(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM sales_returns WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },

  async postReturn(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query(`UPDATE sales_returns SET status = 'posted' WHERE id = $1 AND status = 'draft'`, [id]);
    return { success: result.success, error: result.error };
  },
};
