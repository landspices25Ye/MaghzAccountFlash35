...
export const hrApi = {
  ...
  async getEmployeeById(id: string): Promise<{ success: boolean; data?: Employee; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('SELECT e.*, d.name as department_name FROM employees e LEFT JOIN departments d ON e.department_id = d.id WHERE e.id = $1 LIMIT 1', [id]);
  ...
  async updateEmployee(id: string, data: Partial<Omit<Employee, 'id' | 'companyId'>>): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const fields: string[] = [];
    ...
    values.push(id);
    const result = await adapter.query(`UPDATE employees SET ${fields.join(', ')} WHERE id = $${idx}`, values);
    return { success: result.success, error: result.error };
  },

  async deleteEmployee(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM employees WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },
  ...
  async createPayrollRun(data: Omit<PayrollRun, 'id'>): Promise<{ success: boolean; id?: string; error?: string }> {
    ...
    const tx = await adapter.transaction([
      { sql: `INSERT INTO payroll_runs (company_id, month, year, total_amount, status, notes) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
        params: [data.companyId, data.month, data.year, data.totalAmount, data.status, data.notes] },
    ]);
  ...
  async postPayrollRun(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query("UPDATE payroll_runs SET status = 'posted' WHERE id = $1", [id]);
    return { success: result.success, error: result.error };
  },
  ...
  async updateLeaveStatus(id: string, status: Leave['status'], approvedBy?: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query(
      'UPDATE leaves SET status = $1, approved_by = $2, approved_at = $3 WHERE id = $4',
      [status, approvedBy || null, status === 'approved' ? new Date().toISOString() : null, id]
    );
    return { success: result.success, error: result.error };
  },

  async deleteLeave(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM leaves WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },
  ...
  async updateEndOfServiceStatus(id: string, status: EndOfService['status']): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('UPDATE end_of_service SET status = $1 WHERE id = $2', [status, id]);
    return { success: result.success, error: result.error };
  },

  async deleteEndOfService(id: string): Promise<{ success: boolean; error?: string }> {
    const adapter = await adapterPromise;
    const result = await adapter.query('DELETE FROM end_of_service WHERE id = $1', [id]);
    return { success: result.success, error: result.error };
  },
};
