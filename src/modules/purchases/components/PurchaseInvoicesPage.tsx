...
export const PurchaseInvoicesPage: React.FC = () => {
  ...
  const { invoices, isLoading, create } = usePurchaseInvoices(activeCompany?.id || '');
  const [postingId, setPostingId] = useState<string | null>(null);
  const [isOpen, setIsOpen] = useState(false);
  const [lines, setLines] = useState<InvoiceLine[]>([{ productName: '', quantity: 1, unitPrice: 0, total: 0 }]);
  const [header, setHeader] = useState({ supplier: '', date: new Date().toISOString().split('T')[0], vatRate: 5 });
  ...
  const handleCreate = async () => {
    if (!header.supplier || lines.length === 0 || !activeCompany?.id) return;
    const invNumber = `PINV-2024-${String(invoices.length + 1).padStart(4, '0')}`;
    const newInvoice: Omit<PurchaseInvoice, 'id'> = {
  ...
  const columns = [
    ...
    { key: 'subtotal', header: 'المجموع', align: 'right' as const, render: (row: PurchaseInvoice) => Number(row.subtotal).toLocaleString('ar-SA') },
    { key: 'vatAmount', header: 'الضريبة', align: 'right' as const, render: (row: PurchaseInvoice) => Number(row.vatAmount).toLocaleString('ar-SA') },
    ...
    { key: 'status', header: 'الحالة', render: (row: PurchaseInvoice) => {
      const labels: Record<string, string> = { draft: 'مسودة', posted: 'مرحّلة', paid: 'مدفوعة', partially_paid: 'جزئي', cancelled: 'ملغاة' };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card>
        <Table<PurchaseInvoice> data={invoices} columns={columns} keyExtractor={(row, i) => row.id || String(i)} isLoading={isLoading} emptyMessage="لا توجد فواتير" />
      </Card>
