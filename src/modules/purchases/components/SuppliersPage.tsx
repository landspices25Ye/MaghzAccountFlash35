...
export const SuppliersPage: React.FC = () => {
  ...
  const { suppliers, isLoading, create } = useSuppliers(activeCompany?.id || '');
  ...
  const columns = [
    ...
    { key: 'balance', header: 'الرصيد', align: 'right' as const, render: (row: Supplier) => Number(row.balance).toLocaleString('ar-SA', { minimumFractionDigits: 2 }) },
  ];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">الموردين</h1>
            <p className="text-slate-500 dark:text-slate-400 text-sm">إدارة الموردين وبياناتهم</p>
          </div>
        </div>
